...
def get_friends_for_current_user():
    ...
    friends = get_friends_for_user(current_user)
    print(f"Friends List: {friends}")  # Debugging output
...
def get_friends_for_user(user: str):
    friend_mappings = frappe.db.get_all(
        "Friend Mapping",
        fields=["a", "b", "a.full_name as afn", "b.full_name as bfn", "a.user_image as aui", "b.user_image as bui"],
        or_filters={"a": user, "b": user},
    )

    print(f"Friend Mappings from DB: {friend_mappings}")  # Debugging output
    ...
    print(f"Final Friends List: {friends_list}")  # Debugging output
    return friends_list
# @frappe.whitelist(allow_guest=True)
# def get_friends_for_current_user():
# 	current_user = frappe.session.user
